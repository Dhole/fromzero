...
// Constants
static Obj *True = &(Obj){ TTRUE, 0, { .value = 0 }};
static Obj *Nil = &(Obj){ TNIL, 0, { .value = 0 }};
static Obj *Dot = &(Obj){ TDOT, 0, { .value = 0 }};
static Obj *Cparen = &(Obj){ TCPAREN, 0, { .value = 0 }};
...
static Obj *alloc(void *root, int type, size_t size) {
    ...
    // Allocate the object.
    Obj *obj = (Obj *) ((uint8_t *) memory + mem_nused);
    obj->type = type;
    obj->size = size;
    mem_nused += size;
    return obj;
