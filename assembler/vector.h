...
error_t vector_init(vector_t *v, size_t elem_size, uint16_t init_capacity);
vector_t * vector_new(size_t elem_size, uint16_t init_capacity);
error_t vector_resize(vector_t *v, size_t new_capacity);
void * vector_get(vector_t *v, size_t index);
void vector_clear(vector_t *v, elem_free_fn_t *elem_free_fn);
void vector_free(vector_t *v, elem_free_fn_t *elem_free_fn);
void vector_delete(vector_t *v, elem_free_fn_t *elem_free_fn);
error_t vector_push(vector_t *v, void *elem);
error_t vector_pop_front(vector_t *v, void *elem, elem_free_fn_t *elem_free_fn);
